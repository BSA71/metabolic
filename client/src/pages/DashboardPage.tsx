...
export function DashboardPage() {
  ...
  if (data && !data.program) return <div className="rounded-3xl border border-yellow-200 bg-yellow-50 p-6 text-yellow-900"><h1 className="text-xl font-bold">No active program yet</h1><p className="mt-2 text-sm">Your account exists, but there is no active program or daily log attached to it yet. Seeded demo data belongs to <code>user@metabolic.local</code>.</p></div>;
  if (data && data.program && !data.summary) return <div className="rounded-3xl border border-blue-200 bg-blue-50 p-6 text-blue-900"><h1 className="text-xl font-bold">No daily log for today</h1><p className="mt-2 text-sm">You have an active program, but no daily log has been created for today yet.</p></div>;
